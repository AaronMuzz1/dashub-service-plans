...
import { actorFromUser, canClaimPlan, canCreateDealerPlan, canEditPlan, canManageTemplate, canViewPlan, isDashubAdmin } from "../domain/permissions.js";
import { activateQuote, cancelPlan, claimNextService, markWeeklyPayoutPaid, receiveDealerInvoice, recordTopUp, resolveInvoiceException, scheduleWeeklyPayouts, simulateScheduledPayment } from "../domain/operations.js";
...
export function createServicePlans({ repository, serviceSource, integrations = sandboxIntegrations }) {
  ...
  return {
    ...
    schedulePayouts(actor) {
      permitted(isDashubAdmin(actor));
      const result = scheduleWeeklyPayouts(repository.list(), repository.listPayouts(), context(actor));
      if (result.created.length) repository.putMany(result.plans, result.payouts);
      return result.created;
    },
    markPayoutPaid(actor, payoutId) {
      permitted(isDashubAdmin(actor));
      const result = markWeeklyPayoutPaid(repository.list(), repository.listPayouts(), payoutId, context(actor));
      repository.putMany(result.plans, result.payouts);
    ...
    finance(actor, today) {
      permitted(isDashubAdmin(actor));
      return buildFinanceOverview(repository.list(), repository.listPayouts(), today);
    },
