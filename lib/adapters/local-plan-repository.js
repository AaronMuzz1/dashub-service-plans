...
const empty = () => ({ plans: [], templates: [], companies: [], users: [], payouts: [], adjustments: [] });
...
export function createLocalPlanRepository(storage) {
  ...
  return {
    ...
    putMany: (plans, payouts, adjustments) => transact((data) => {
      plans.forEach((plan) => replace(data.plans, plan));
      if (payouts) data.payouts = payouts;
      if (adjustments) data.adjustments = adjustments;
      return plans;
    }),
    ...
    listAdjustments: () => read().adjustments ?? [],
